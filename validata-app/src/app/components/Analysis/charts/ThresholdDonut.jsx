...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-md text-xs">
      ...
      <p className="text-slate-600">{value} measurement{value !== 1 ? 's' : ''}</p>
    </div>
  );
};
    ...
    <div className="relative">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-center">
          <p className="text-3xl font-bold text-slate-800">{percentage.toFixed(1)}%</p>
          <p className="text-xs text-slate-500 mt-0.5">within ±{threshold}°</p>
